...
//! Food-service platform engine — binary entry.
//!
//! `app-mediakit-foodservice serve --content-dir <dir>` renders pages
//! from typed section-manifests. Per-instance config via env/args.
//! See `src/lib.rs` for the architecture overview.
...
use app_mediakit_foodservice::config::Config;
...
#[derive(Parser)]
#[command(name = "app-mediakit-foodservice", version, about)]
struct Cli {
    #[command(subcommand)]
    command: Command,
}
...
#[derive(Subcommand)]
enum Command {
    /// Run the food-service engine HTTP server.
    Serve {
        /// Directory of page manifests (`<dir>/<slug>/page.yaml`).
        #[arg(long, env = "SERVICE_FOODSERVICE_CONTENT_DIR")]
        content_dir: PathBuf,

        /// Persistent state dir (AI-proposal review queue).
        #[arg(
            long,
            env = "SERVICE_FOODSERVICE_STATE_DIR",
            default_value = "/var/lib/local-foodservice/state"
        )]
        state_dir: PathBuf,
        ...
        #[arg(long, env = "SERVICE_FOODSERVICE_SITE_TITLE")]
        site_title: Option<String>,

        /// Optional external DTCG tokens.css (Style-Dictionary output).
        #[arg(long, env = "SERVICE_FOODSERVICE_TOKENS_CSS")]
        ...
        #[arg(long, env = "SERVICE_FOODSERVICE_ENABLE_MCP")]
        enable_mcp: bool,
    },
}
...
async fn serve(cfg: Config) -> Result<()> {
    ...
    brand.google_verify = std::env::var("SERVICE_FOODSERVICE_GOOGLE_VERIFY").ok();
