...
//!
//! Agent-first authoring surface: AI authors discover the typed section
//! vocabulary, read pages, validate manifests, and propose pages. Proposals
//! stage to the human review queue — never auto-committed (SYS-ADR-10);
//! a human approves (F12) before anything persists (SYS-ADR-19).
//!
...
fn initialize() -> Value {
    json!({
        ...
        "serverInfo": { "name": "app-mediakit-foodservice", "version": env!("CARGO_PKG_VERSION") }
    })
}
    ...
    json!({ "tools": [
        {
            ...
            "description": "List the typed section vocabulary a page may be composed from.",
        ...
        },
        {
            ...
            "description": "Validate a page manifest (YAML) against the section contract WITHOUT staging it.",
            "inputSchema": {
                "type": "object",
        ...
        {
            "name": "propose_page",
            "description": "Propose a page manifest. Staged to the human review queue. Per SYS-ADR-10 NOT published until a human approves (F12).",
            "inputSchema": {
                "type": "object",
...
async fn tools_call(state: &AppState, params: &Value) -> Result<Value, (i32, String)> {
    ...
    let text = match name {
        ...
        "propose_page" => {
            ...
            format!(
                "Proposal staged as '{id}'. Per SYS-ADR-10 NOT published — a human must approve (F12) before it persists."
            )
        }
...
fn resources_list(state: &AppState) -> Value {
    let resources: Vec<Value> = content::list_slugs(&state.content_dir)
        ...
        .map(|slug| {
            json!({
                "uri": format!("foodservice://page/{slug}"),
                "name": slug,
