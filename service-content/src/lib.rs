...
mod entity_hints;
mod er;
/// Exposed (not just `mod`) so `tests/http_test.rs` can build a real `LbugGraphStore`
/// over a tempdir and drive it directly for the concurrency-safety tests.
pub mod graph;
/// Exposed so `tests/http_test.rs` can build a real `Router` via `http::router` and
/// drive it with `tower::ServiceExt::oneshot` — mirrors the `slm-doorman-server` crate's
/// own `pub mod http` convention (see that crate's `lib.rs`).
pub mod http;
/// Exposed so `tests/http_test.rs` can construct real signed `X-Foundry-Capability`
/// headers and register test peers via `PairingStore`/`PairingRecord`.
pub mod pairing;
#[cfg(test)]
mod pipeline_tests;
mod taxonomy;
/// SYS-ADR-19 write-governance checkpoint. Exposed so `tests/http_test.rs` can
/// construct a `WriteGovernanceDispatcher` with a mock `WriteVerdictVerifier`
/// (real `ssh-keygen -Y verify` isn't exercisable from an automated test).
pub mod write_governance;

/// Test helpers — factory functions shared across integration test files.
///
/// Only useful to `#[cfg(test)]` consumers (the `tests/` integration-test crate); the
/// module itself is always present at the crate boundary, same convention as
/// `slm-doorman-server`'s `test_helpers` module.
pub mod test_helpers {
    use crate::graph::{GraphStore, LbugGraphStore};
    use crate::http::HttpState;
    use crate::pairing::{
        CapabilityPayload, InterfaceAuditLog, NonceCache, PairingKeypair, PairingRecord,
        PairingStore,
    };
    use crate::write_governance::{
        WriteGovernanceDispatcher, WriteGovernanceError, WriteVerdictVerifier,
    };
    ...
    /// Permissive test verifier for `WriteGovernanceDispatcher` — accepts any
    /// signature equal to `TRUSTED_TEST_SIGNATURE`, for any reviewer identity, in
    /// the correct namespace. Real `ssh-keygen -Y verify` isn't exercisable from
    /// an automated test; tests needing to exercise rejection paths build their
    /// own stricter `WriteVerdictVerifier` impl directly (the trait is `pub`).
    pub const TRUSTED_TEST_SIGNATURE: &str = "TRUSTED-TEST-SIGNATURE";

    #[derive(Debug)]
    pub struct PermissiveTestVerifier;

    impl WriteVerdictVerifier for PermissiveTestVerifier {
        fn verify(
            &self,
            _body: &str,
            signature_pem: &str,
            _reviewer_identity: &str,
            namespace: &str,
        ) -> Result<(), WriteGovernanceError> {
            if signature_pem == TRUSTED_TEST_SIGNATURE
                && namespace == crate::write_governance::WRITE_GOVERNANCE_NAMESPACE
            {
                Ok(())
            } else {
                Err(WriteGovernanceError::VerifySignature(
                    "PermissiveTestVerifier rejected".into(),
                ))
            }
        }
    }

    /// Build a real `LbugGraphStore` over a fresh temp directory. Each call gets a
    /// distinct directory (nanosecond-timestamp-suffixed, same convention as
    /// `slm-doorman-server`'s own `test_helpers`) so parallel tests never collide —
    /// unlike `pipeline_tests.rs`'s single-test `process::id()` pattern, this file has
    /// dozens of tests running concurrently. Deliberately uses `std::env::temp_dir()`
    /// rather than the `tempfile` crate: this module is NOT `#[cfg(test)]`-gated (so
    /// `tests/http_test.rs`, a separate compilation unit, can see it), which means it
    /// compiles as part of the normal library build too — where `tempfile` (a
    /// dev-dependency) is unavailable. Returns the store plus its base directory path
    /// (callers derive `graph_dir`/`corpus_dir` subpaths from it); directories are not
    /// auto-cleaned on drop, matching this crate's existing test convention.
    pub fn temp_graph_store() -> (Arc<dyn GraphStore>, std::path::PathBuf) {
        let base =
            std::env::temp_dir().join(format!("service-content-http-test-{}", unique_suffix()));
        std::fs::create_dir_all(&base).expect("create temp base dir");
        let store =
            LbugGraphStore::new(base.join("lbug").to_str().unwrap()).expect("open temp lbug store");
        store.init_schema().expect("init_schema");
        (Arc::new(store), base)
    }

    /// Nanosecond-timestamp-based uniqueness suffix — no `rand`/`uuid` dependency
    /// needed just for temp-dir names; monotonic-enough within a single test process.
    ...
    /// Build a minimal `HttpState` for router-level tests: no Doorman/ontology
    /// wiring (empty strings — routes that need them are not under test here),
    /// fresh `PairingStore`/`PairingKeypair` under the given `graph_dir`. Both
    /// `graph_dir` and `corpus_dir` must already exist as real directories
    /// (e.g. a `tempfile::TempDir`'s own path, or a subdir created under one) —
    /// `PairingKeypair::load_or_generate` and `ingest_document`'s corpus-file
    /// write both fail if their parent directory is missing.
    pub fn test_http_state(
        graph: Arc<dyn GraphStore>,
        graph_dir: &str,
        corpus_dir: &str,
    ) -> Arc<HttpState> {
        test_http_state_with_write_verifier(
            graph,
            graph_dir,
            corpus_dir,
            Arc::new(PermissiveTestVerifier),
        )
    }

    /// Same as `test_http_state`, but lets the caller supply a custom
    /// `WriteVerdictVerifier` — used by tests that need to exercise
    /// signature-rejection paths on `POST /v1/graph/verdict`.
    pub fn test_http_state_with_write_verifier(
        graph: Arc<dyn GraphStore>,
        graph_dir: &str,
        corpus_dir: &str,
        verifier: Arc<dyn WriteVerdictVerifier>,
    ) -> Arc<HttpState> {
        let pairing_store = PairingStore::load(graph_dir).expect("pairing store");
        let pairing_key = PairingKeypair::load_or_generate(graph_dir).expect("pairing key");
        let pending_dir = std::path::Path::new(graph_dir).join("pending-writes");
        Arc::new(HttpState {
            graph,
            doorman_endpoint: String::new(),
            ontology_dir: String::new(),
            corpus_dir: corpus_dir.to_string(),
            graph_dir: graph_dir.to_string(),
            pairing_store: Mutex::new(pairing_store),
            nonce_cache: NonceCache::new(),
            pairing_key,
            capability_audit: InterfaceAuditLog::new(graph_dir),
            write_governance: WriteGovernanceDispatcher {
                verifier,
                pending_dir,
            },
        })
    ...
    /// Generate a fresh, standalone Ed25519 keypair representing an arbitrary
    /// "forwarding peer" — deliberately NOT derived from the server's own
    /// `PairingKeypair` (that key is for issuing OUR `/v1/pair` tokens, not for
    /// signing an incoming peer's capability headers; a real forwarding peer
    /// always uses its own separate key, registered via `/v1/pair` at pairing time).
    pub fn test_peer_keypair(seed_byte: u8) -> SigningKey {
        SigningKey::from_bytes(&[seed_byte; 32])
    }

    /// Register a test peer's public key in the given `HttpState`'s `PairingStore`,
    /// so `capability_gate`'s `find_by_instance` lookup resolves it.
    pub fn register_test_peer(
        ...
        let rec = PairingRecord {
            public_key,
            issuer: from_instance.to_string(),
            peer_type: "orchestration".to_string(),
            role: role.to_string(),
            archive_scope,
            node_label: from_instance.to_string(),
            paired_on: Utc::now().to_rfc3339(),
            nonce: format!("register-{from_instance}"),
        };
        state
            .pairing_store
            .lock()
    ...
    /// Build a signed `X-Foundry-Capability` header value for the given peer key.
    /// Always a direct grant (`forwarded_for: None`) — use
    /// `make_forwarded_capability_token` to build a forward-claim token.
    pub fn make_capability_token(
        signing_key: &SigningKey,
        from_instance: &str,
        user_scope: &str,
        archive_scope: Vec<String>,
    ...
    /// Build a signed `X-Foundry-Capability` header value asserting the peer is
    /// forwarding a capability on behalf of `forwarded_for` — grant-vs-forward
    /// (`BRIEF-datagraph-tenant-isolation.md`, Session 4).
    pub fn make_forwarded_capability_token(
        signing_key: &SigningKey,
        from_instance: &str,
        user_scope: &str,
        archive_scope: Vec<String>,
        ...
        make_capability_token_inner(
            ...
            archive_scope,
            expiry,
            Some(forwarded_for.to_string()),
        )
    }

    fn make_capability_token_inner(
        signing_key: &SigningKey,
        from_instance: &str,
        user_scope: &str,
        archive_scope: Vec<String>,
        ...
        let payload = CapabilityPayload {
            ...
            archive_scope,
            nonce: format!("test-nonce-{}", unique_suffix()),
        ...
        let sig: Signature = signing_key.sign(pb64.as_bytes());
...
pub fn run() -> NotifyResult<()> {
    ...
    // Startup failures here return Err (propagated by `?` further below)
    // instead of calling process::exit(1) directly. In a standalone binary
    // (service-content/src/main.rs) that distinction is invisible — a
    // returned Err still exits the process with a non-zero code via the
    // default Termination impl. It matters a great deal in os-totebox's
    // bundled context (os-totebox/src/main.rs): process::exit() is
    // process-wide, so a startup failure here used to kill the Doorman's
    // thread too, directly undermining that file's own stated design intent
    // ("a panic or long block in one service's thread can never starve the
    // other's runtime"). Fixed 2026-07-30 alongside the SIGTERM/graceful-
    // shutdown work — see BRIEF-os-totebox-platform.md.
    if !Path::new(&corpus_dir).exists() {
        fs::create_dir_all(&corpus_dir).map_err(|e| {
            eprintln!("[FATAL] Cannot create corpus dir {:?}: {e}", corpus_dir);
            notify::Error::generic(&format!("cannot create corpus dir {corpus_dir:?}: {e}"))
        })?;
    }
    if !Path::new(&crm_dir).exists() {
        fs::create_dir_all(&crm_dir).map_err(|e| {
            eprintln!("[FATAL] Cannot create CRM dir {:?}: {e}", crm_dir);
            notify::Error::generic(&format!("cannot create CRM dir {crm_dir:?}: {e}"))
        })?;
    }
    ...
    fs::create_dir_all(&graph_dir).map_err(|e| {
        ...
        notify::Error::generic(&format!("cannot create graph dir {graph_dir:?}: {e}"))
    })?;
    ...
    // `run()` is synchronous with no top-level tokio runtime, so a dedicated
    // thread with its own minimal runtime catches the signal — the same
    // pattern the HTTP server thread below already uses for a different
    // reason (nested-runtime avoidance). Multiple independent SIGTERM
    // listeners in one process are a supported, safe pattern (the underlying
    // `signal-hook-registry` crate — a transitive tokio dependency — is a
    // registry that dispatches to every listener, not a single-owner slot);
    // `run_server` below installs its own independent listener for the same
    // signal to drive its own axum graceful shutdown.
    //
    // On SIGTERM: sets `shutdown`, which the main watcher loop below checks
    // after every `rx.recv_timeout` wakeup (worst case ~3s later, the
    // shortest poll interval) and stops on — no new CORPUS files are picked
    // up, `run()` returns, and `tier_a_tx` (a local var here) drops with it,
    // closing that channel so the Tier A training worker thread's `for job
    // in tier_a_rx` loop finishes its current job (if any) and exits too.
    // `graph_store`'s last `Arc` reference dropping (across this thread, the
    // HTTP thread once its own graceful shutdown completes, and the Tier A
    // worker thread) triggers LadybugDB's checkpoint-on-close (confirmed via
    // `lbug`'s own `force_checkpoint_on_close`/`auto_checkpoint` defaults,
    // both `true`) — see BRIEF-os-totebox-platform.md's SIGTERM section for
    // the full design note. This does not (yet) explicitly join the HTTP or
    // Tier A worker threads before `run()` returns — a known, documented
    // remaining gap, not silently assumed complete; see the BRIEF.
    //
    // The channel below (normally created just before `notify::recommended_
    // watcher` further down) is created here instead, earlier, so the signal
    // thread can hold a clone of `tx` and push a synthetic wake-up event the
    // instant SIGTERM arrives. Without this, the watcher loop only notices
    // `shutdown` at its next scheduled `rx.recv_timeout` wakeup — up to 30s
    // away when idle (confirmed empirically 2026-07-29: a live SIGTERM test
    // in the seL4 guest showed the process still running 20s after signal,
    // waiting on exactly this poll interval, even though both HTTP servers
    // had already drained and shut down cleanly within milliseconds). A 30s
    // worst-case shutdown latency risks losing to a stricter orchestrator
    // grace period (e.g. Kubernetes' 30s default) before the graceful path
    // even starts — the synthetic wake-up event closes that gap to
    // effectively instant, matching the HTTP servers' own latency.
    ...
    let shutdown = Arc::new(AtomicBool::new(false));
    {
        ...
        thread::spawn(move || {
            let rt = match tokio::runtime::Runtime::new() {
                Ok(rt) => rt,
                Err(e) => {
                    eprintln!("[SYSTEM] failed to build signal-watcher runtime: {e} — SIGTERM will not be handled gracefully");
                    return;
                }
            };
            rt.block_on(async {
                match tokio::signal::unix::signal(tokio::signal::unix::SignalKind::terminate()) {
                    Ok(mut sigterm) => {
                        sigterm.recv().await;
                        println!("[SYSTEM] SIGTERM received — draining and shutting down gracefully...");
                        shutdown.store(true, Ordering::SeqCst);
                        // Wake the watcher loop immediately instead of waiting for its
                        // next scheduled poll — see the comment above this block.
                        let _ = wake_tx.send(Ok(Event::new(EventKind::Other)));
                    }
                    Err(e) => {
                        eprintln!("[SYSTEM] failed to install SIGTERM handler: {e} — SIGTERM will not be handled gracefully");
                    }
                }
            });
        });
    ...
    // `tx`/`rx` were created earlier (see the SIGTERM handling comment above)
    // so the signal thread could hold a clone of `tx` for its wake-up event.
    ...
    loop {
        if shutdown.load(Ordering::SeqCst) {
            println!("[SYSTEM] shutdown requested — no new CORPUS files will be picked up, exiting watcher loop.");
            break;
        }
...
fn call_tier_0_gliner(corpus_text: &str, domain_id: Option<&str>) -> GlinerOutcome {
    call_tier_0_gliner_at(GLINER_ENDPOINT, corpus_text, domain_id)
}

/// Same as `call_tier_0_gliner` with an injectable endpoint — production
/// code always goes through `call_tier_0_gliner` (fixed at `GLINER_ENDPOINT`);
/// this split exists so tests can point at a mock server instead of a live
/// GLiNER instance.
fn call_tier_0_gliner_at(
    endpoint: &str,
    corpus_text: &str,
    ...
    let primary_domain = domain_id.unwrap_or("projects");
    let entities = match run_gliner_domain(&client, endpoint, &chunks, primary_domain) {
        Ok(e) => e,
        Err(outcome) => return outcome,
    };

    if !entities.is_empty() {
        return finish_gliner_found(entities, chunks.len());
    }

    // Empty result under the hardcoded default domain is a genuine full-miss
    // mechanism, not necessarily "no entities present" — GLiNER's zero-shot
    // signal is label wording, so unmatched domain vocabulary can silently
    // miss real content (root-caused 2026-07-20, BRIEF-flow-quality-audit.md).
    // Retry once against "corporate" before giving up, but never override an
    // explicit caller-supplied domain_id — that choice is intentional.
    if domain_id.is_none() && primary_domain != "corporate" {
        match run_gliner_domain(&client, endpoint, &chunks, "corporate") {
            Ok(e) if !e.is_empty() => return finish_gliner_found(e, chunks.len()),
            Ok(_) => {}
            Err(outcome) => return outcome,
        }
    }
...
/// was reachable and answered normally for every chunk.
fn run_gliner_domain(
    client: &reqwest::blocking::Client,
    endpoint: &str,
    chunks: &[&str],
    domain: &str,
) -> Result<Vec<serde_json::Value>, GlinerOutcome> {
    let url = format!("{endpoint}/v1/extract");
    let mut seen: std::collections::HashSet<String> = std::collections::HashSet::new();
    ...
    for chunk in chunks {
        ...
        let resp = match client.post(&url).json(&body).send() {
            ...
            Err(e) => {
                ...
                return Err(GlinerOutcome::Unavailable);
            }
        };
...
fn finish_gliner_found(entities: Vec<serde_json::Value>, chunk_count: usize) -> GlinerOutcome {
    if chunk_count > 1 {
        eprintln!(
    ...
    let flush_tier_a = |tier_a: &Option<Vec<serde_json::Value>>, reason: &str| -> ExtractResult {
        if let Some(ta_ents) = tier_a {
            if !ta_ents.is_empty() {
                ...
                match crate::write_governance::write_or_hold(
                    graph_store,
                    effective_module_id,
                    &ge,
                    Some(worm_id),
                    ...
                    Ok(crate::write_governance::WriteOutcome::WrittenDirect(n)) => {
                        println!("  -> [TIER-A] {} entities written ({}).", n, reason);
                        return ExtractResult::Success;
                    }
                    Ok(crate::write_governance::WriteOutcome::HeldPending(write_id)) => {
                        println!(
                            "  -> [TIER-A] {} entities held pending write-governance verdict \
                             (write_id={}, {}).",
                            ge.len(),
                            write_id,
                            reason
    ...
    match res {
        Ok(response) => {
                    ...
                    _ => {
                        ...
                        if let Some(ta_ents) = &tier_0_entities {
                            if !ta_ents.is_empty() {
                                ...
                                if let Ok(outcome) = crate::write_governance::write_or_hold(
                                    graph_store,
                                    effective_module_id,
                                    &ge,
                                    Some(worm_id),
                                    ...
                                    match outcome {
                                        crate::write_governance::WriteOutcome::WrittenDirect(n) => {
                                            println!(
                                                "  -> [TIER-A] {} entities written (Tier B transient; module: {}).",
                                                n, effective_module_id
                                            );
                                        }
                                        crate::write_governance::WriteOutcome::HeldPending(
                                            write_id,
                                        ) => {
                                            println!(
                                                "  -> [TIER-A] {} entities held pending write-governance verdict \
                                                 (write_id={}, Tier B transient; module: {}).",
                                                ge.len(),
                                                write_id,
                                                effective_module_id
                                            );
            ...
            *tier_b_used = true;
            ...
            let graph_write = crate::write_governance::write_or_hold(
                graph_store,
                effective_module_id,
                &graph_entities,
                Some(worm_id),
            ...
            let held_pending = match graph_write {
                Ok(crate::write_governance::WriteOutcome::WrittenDirect(n)) => {
                    println!(
                ...
                Ok(crate::write_governance::WriteOutcome::HeldPending(write_id)) => {
                    println!(
                        "  -> [GRAPH] {} entities held pending write-governance verdict \
                         (write_id={}, module: {}).",
                        graph_entities.len(),
                        write_id,
                        effective_module_id
            ...
            // The CRM ledger references entities that must already exist in the
            // graph ("keeps stores consistent") — when write-governance holds the
            // graph write pending, the entities aren't confirmed in the graph yet
            // (may still be rejected), so the CRM write is skipped too. It is not
            // retroactively written on verdict-accept (a real gap, not silently
            // pretended away) — CRM is this pipeline's legacy secondary output,
            // not the primary graph store the checkpoint exists to protect.
            if held_pending {
                println!("  -> [CRM] Skipped — graph write held pending write-governance verdict.");
                return ExtractResult::Success;
            }
...
mod tests {
    ...
    // Root-caused 2026-07-20 (BRIEF-flow-quality-audit.md): the hardcoded
    // "projects" default domain silently misses generic corporate content
    // because GLiNER's zero-shot signal is label wording, not the presence
    // of entities. Confirmed live 2026-07-27 against the real service
    // ("Jennifer Woodfine met with PointSav Digital Systems today." returns
    // zero entities under domain_id=projects, a real Person under
    // domain_id=corporate) before this fix landed.

    #[tokio::test(flavor = "multi_thread")]
    async fn call_tier_0_gliner_falls_back_to_corporate_domain_on_empty_default() {
        ...
        Mock::given(method("POST"))
            .and(path("/v1/extract"))
            .and(body_string_contains("\"domain_id\":\"projects\""))
            .respond_with(
        ...
        Mock::given(method("POST"))
            .and(path("/v1/extract"))
            .and(body_string_contains("\"domain_id\":\"corporate\""))
            .respond_with(ResponseTemplate::new(200).set_body_json(serde_json::json!({
                "entities": [{"entity_name": "Jennifer Woodfine", "classification": "Person"}]
            })))
            .mount(&mock_server)
            .await;

        let endpoint = mock_server.uri();
        let text = "Jennifer Woodfine met with PointSav Digital Systems today.".to_string();
        let outcome =
            tokio::task::spawn_blocking(move || call_tier_0_gliner_at(&endpoint, &text, None))
                .await
                .unwrap();
        ...
        match outcome {
            GlinerOutcome::Found(entities) => {
                ...
                assert_eq!(entities[0]["entity_name"], "Jennifer Woodfine");
            }
            GlinerOutcome::Empty => panic!("expected Found after corporate fallback, got Empty"),
            GlinerOutcome::Unavailable => {
                panic!("expected Found after corporate fallback, got Unavailable")
            }
        }
    }

    #[tokio::test(flavor = "multi_thread")]
    async fn call_tier_0_gliner_does_not_fall_back_when_caller_specifies_domain() {
        use wiremock::matchers::{body_string_contains, method, path};
        use wiremock::{Mock, MockServer, ResponseTemplate};

        let mock_server = MockServer::start().await;

        // Only "projects" is ever mocked — if the fallback fired despite an
        // explicit caller domain_id, the (unmocked) "corporate" request
        // would 404 and this test would see Unavailable instead of Empty.
        Mock::given(method("POST"))
            .and(path("/v1/extract"))
            .and(body_string_contains("\"domain_id\":\"projects\""))
            .respond_with(
        ...
        let endpoint = mock_server.uri();
        let text = "Some content with no matching entities.".to_string();
        let outcome = tokio::task::spawn_blocking(move || {
            call_tier_0_gliner_at(&endpoint, &text, Some("projects"))
        })
        .await
        .unwrap();
        ...
        match outcome {
            GlinerOutcome::Empty => {}
            GlinerOutcome::Found(_) => panic!("expected Empty, got Found"),
            GlinerOutcome::Unavailable => {
                panic!("expected Empty — Unavailable means the fallback wrongly fired against an unmocked domain")
            }
        }
    }
}
