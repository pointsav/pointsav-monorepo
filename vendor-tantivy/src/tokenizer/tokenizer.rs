...
impl Default for TextAnalyzer {
    fn default() -> TextAnalyzer {
        TextAnalyzer::builder(EmptyTokenizer).build()
    }
}

// Blanket impl removed: `impl<T: Tokenizer + Clone> From<T> for TextAnalyzer`
// triggered E0119 under rustc 1.85+ coherence checks. Replaced with concrete
// impls for each tokenizer type registered via TokenizerManager.
macro_rules! impl_text_analyzer_from {
    ($($t:ty),* $(,)?) => {
        $(impl From<$t> for TextAnalyzer {
...
impl_text_analyzer_from!(
    EmptyTokenizer,
    crate::tokenizer::RawTokenizer,
    crate::tokenizer::WhitespaceTokenizer,
    crate::tokenizer::SimpleTokenizer,
    crate::tokenizer::RegexTokenizer,
    crate::tokenizer::NgramTokenizer,
    crate::tokenizer::FacetTokenizer,
);
