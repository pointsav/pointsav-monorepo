...
use system_vm_fleet_types::{VmMeta, VmRecord, VmState};
...
mod tests {
    ...
    #[test]
    fn read_vm_meta_returns_correct_fields_when_file_present() {
        let dir = std::env::temp_dir().join("vm-meta-monitor-test");
        std::fs::create_dir_all(&dir).unwrap();
        let vm_id = "vm-monitor-meta-test";
        let meta = VmMeta {
            vm_id: vm_id.to_string(),
        ...
        let meta_path = dir.join(format!("{vm_id}.meta.json"));
        std::fs::write(&meta_path, serde_json::to_vec(&meta).unwrap()).unwrap();

        let fake_socket = dir.join(format!("{vm_id}.monitor.sock"));
        let (vm_type, ram, vcpu, started) = read_vm_meta(&fake_socket, vm_id);
        assert_eq!(vm_type, "VmTotebox");
        assert_eq!(ram, 4096);
        assert_eq!(vcpu, 4);
    ...
    #[test]
    fn read_vm_meta_falls_back_to_stubs_when_file_absent() {
        let fake_socket =
            std::path::Path::new("/tmp/nonexistent-vm-monitor-meta-dir/vm-x.monitor.sock");
        let (vm_type, ram, vcpu, started) = read_vm_meta(fake_socket, "vm-x");
        assert_eq!(vm_type, "unknown");
        assert_eq!(ram, 0);
        assert_eq!(vcpu, 0);
