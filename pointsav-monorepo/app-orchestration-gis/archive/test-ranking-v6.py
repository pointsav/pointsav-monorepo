...
def evaluate_tier(anchor_id, nhw, nwh, nhc, nhe, cont):
    ...
    # T3: Anchor + HW + WH + (Hospital OR University)
    # T2: Anchor + (HW or WH) + (Hospital OR University)
    # T1: Anchor + (HW or WH)
    
    # We must hit ~400 T3/T4 for both continents
    if has_hw and has_wh and has_hc and has_he: return 4
    if has_hw and has_wh and (has_hc or has_he): return 3
    if (has_hw or has_wh) and has_hc and has_he: return 3
