...
def evaluate_tier(anchor_id, nhw, nwh, nhc, nhe, cont):
    ...
    # T4: Apex - Anchor + HW + WH + Civic(Both) OR Anchor + (HW or WH) + Civic(Both)
    # T3: Hub - Anchor + HW + WH OR Anchor + (HW or WH) + Civic(One)
    # T2: Core - Anchor + HW OR Anchor + WH
    # T1: Baseline - Anchor + Civic(One) OR just Anchor alone? 
    # Let's say T1 is Anchor + Civic(One) and we drop standalone anchors.

    if (has_hw and has_wh and has_hc and has_he) or ((has_hw or has_wh) and has_hc and has_he):
        # We need EU to pop at least ~25 T4s. Let's see if this does it.
        return 4
    if (has_hw and has_wh) or ((has_hw or has_wh) and (has_hc or has_he)):
        return 3
    if has_hw or has_wh:
        return 2
    if has_hc or has_he:
    ...
    for r_key, r_roles in REGION_CONFIG.items():
        ...
        for anchor_cid in r_roles.get("anchor", []):
            for pri in locs.get(anchor_cid, []):
                ...
                # Boost civic catchment to 7km in EU to find more universities?
                civic_rad = 7.0 if cont == "EU" else 5.0
                nhc = query_grid(plat, plon, hc_g, civic_rad)
                nhe = query_grid(plat, plon, he_g, civic_rad)
