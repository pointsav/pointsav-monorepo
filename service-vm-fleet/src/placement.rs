...
pub fn select_node(registry: &NodeRegistry, ram_mb: u64) -> Option<NodeId> {
...
mod tests {
    ...
    fn selects_node_with_most_ram() {
        ...
        let result = select_node(&reg, 1024);
    ...
    fn safety_margin_enforced() {
        ...
        assert!(select_node(&reg, 512).is_some());
        // Requesting 513 MB — needs 513 + 512 = 1025 MB — exceeds available
        assert!(select_node(&reg, 513).is_none());
    }

    #[test]
    fn empty_registry_returns_none() {
        let reg = NodeRegistry::new();
        assert_eq!(select_node(&reg, 1024), None);
    }
}
