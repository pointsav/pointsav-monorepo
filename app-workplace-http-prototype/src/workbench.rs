...
async fn move_file(State(state): State<AppState>, Query(q): Query<RenameQuery>) -> Response {
    ...
    // Extract filename and build final path
    ...
    if final_path.exists() {
        return err(StatusCode::CONFLICT, "a file with that name already exists in the destination");
    }

    // Attempt atomic rename; fall back to copy+delete for cross-filesystem moves
    ...
    // Build new URL path: destination url prefix + filename
