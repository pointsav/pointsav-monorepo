...
fn check_throttle() -> bool {
    ...
    if count >= MAX_DAILY_VERIFICATIONS {
        ...
        println!(
            "You have completed your {} verifications for {}.",
            MAX_DAILY_VERIFICATIONS, today
        );
...
fn main() {
    ...
    let mut entity: EntityBundle =
        serde_json::from_str(&content).expect("Failed to parse Entity Bundle");

    // 1. Enhanced TUI Display
    println!(
        "\n[TARGET IDENTITY]: {}",
        entity.display_name.to_uppercase()
    );
    println!("--------------------------------------------------------");
    ...
    let mut search_string = format!(
        "site:linkedin.com/in/ OR site:linkedin.com/company/ \"{}\"",
        entity.display_name
    );
    ...
    let domains = load_options::<SeedDomains, _>(&format!("{}/Domains.json", seed_dir), |data| {
        data.domains.into_iter().map(|d| d.domain_name).collect()
    });
    let themes = load_options::<SeedThemes, _>(&format!("{}/Themes.json", seed_dir), |data| {
        data.themes.into_iter().map(|t| t.name).collect()
    });
    ...
    if trimmed_url == "delete" {
        ...
        println!(
            "\n[SYSTEM] Entity '{}' permanently destroyed. Returning to pool.",
            entity.display_name
        );
        std::process::exit(0);
    } else if trimmed_url == "skip" {
        println!(
            "\n[SYSTEM] Entity '{}' skipped. Returning to pool.",
            entity.display_name
        );
        std::process::exit(0);
    }

    let sel_domain = get_selection(
        "Which DOMAIN (Glossary) does this entity belong to?",
        &domains,
    );
    let sel_theme = get_selection(
        "Which THEME (Concept/Trend) drives their operation?",
        &themes,
    );
    let sel_coa = get_selection(
        "Where do they slot into our CHART OF ACCOUNTS (Categorization)?",
        &coa,
    );
    ...
    println!("\n========================================================");
    println!(
        "✅ [SUCCESS] Entity '{}' mathematically verified and vaulted.",
        entity.display_name
    );
    println!("========================================================");
}
