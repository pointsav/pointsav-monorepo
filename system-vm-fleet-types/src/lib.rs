...
/// Sidecar metadata written by service-vm-fleet at spawn time.
///
/// Stored alongside the disk image as `{vm_id}.meta.json` so that service-vm-host
/// can populate accurate VmRecord fields when polling running QEMU processes.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)]
pub struct VmMeta {
    pub vm_id: String,
    pub vm_type: String,
...
mod tests {
    ...
    #[test]
    fn vm_meta_round_trips_json() {
        let meta = VmMeta {
            vm_id: "vm-totebox-1".to_string(),
            vm_type: "VmTotebox".to_string(),
            ram_alloc_mb: 4096,
            vcpu_count: 4,
            started_at: Utc::now(),
        ...
        let decoded: VmMeta = serde_json::from_str(&json).unwrap();
        assert_eq!(decoded.vm_id, "vm-totebox-1");
        assert_eq!(decoded.vm_type, "VmTotebox");
        assert_eq!(decoded.ram_alloc_mb, 4096);
