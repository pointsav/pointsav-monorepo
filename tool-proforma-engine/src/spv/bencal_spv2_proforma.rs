...
pub fn forecast(pclp1: &[Pclp1Year], wcp: &[WcpYear]) -> Vec<BencalSpv2Year> {
    ...
    let mut _prev_wcp_shares = SPV2_WCP_FOUNDING_BONUS;
    let mut cumulative_cash: f64 = 0.0;

    for y in 0u32..=10 {
        let commission = if y == 0 {
            SPV2_COMMISSION_REBATE_Y0_GROSS
        } else {
            0.0
        };
        ...
        let pclp1_dpu = if (y as usize) < pclp1.len() {
            pclp1[y as usize].dpu
        } else {
            0.0
        };
        ...
        // PCLP 1 NAV
        let pclp1_nav_per_unit = if (y as usize) < pclp1.len() {
            pclp1[y as usize].nav_per_unit
        } else {
            0.0
        };
        ...
        let wcp_value_per_share = if y < 4 {
            ...
            wcp[y as usize]
                ...
                .max(SPV2_WCP_LEVEL3_PROXY)
        } else {
            SPV2_WCP_LEVEL3_PROXY
        };

        // WCP shares held: 600K through Y9; 206K after Y10 sale
        let wcp_shares_held = if y < 10 {
            SPV2_WCP_FOUNDING_BONUS
        } else {
            SPV2_WCP_RESIDUAL_SHARES
        };
        ...
        let tax = if income_before_tax > 0.0 {
            income_before_tax * SPV2_TAX_RATE_CURRENT
        } else {
            0.0
        };
        ...
        _prev_wcp_shares = wcp_shares_held;
        let _ = _prev_wcp_shares;
    }
    years
}
...
mod tests {
    ...
    fn y4_plus_pclp1_distributions() {
        ...
        for y in 4..=10 {
            ...
            assert!(
                (spv2[y].pclp1_distributions - expected).abs() < 1.0,
                "Y{} PCLP1 dist = {} (expected {})",
                y,
                spv2[y].pclp1_distributions,
                expected
            );
        }
    }
    ...
    fn y10_total_return_above_capital() {
        ...
        assert!(
            tr > SPV2_TOTAL_INVESTOR_CAPITAL,
            "Y10 total return ({}) should exceed capital ({})",
            tr,
            SPV2_TOTAL_INVESTOR_CAPITAL
        );
    }
