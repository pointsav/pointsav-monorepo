...
pub fn forecast(spv1: &[BencalSpv1Year], spv2: &[BencalSpv2Year]) -> Vec<BencalMgmtYear> {
    ...
    for y in 0u32..=10 {
        ...
        let tax = if income_before_tax > 0.0 {
            income_before_tax * BM_TAX_RATE_CURRENT
        } else {
            0.0
        };
        ...
        let spv2_nav_lookthrough =
            (s2.pclp1_nav_total + s2.wcp_holding_fv) * BM_LOOKTHROUGH_STAKE_SPV2;
...
mod tests {
    ...
    fn lookthrough_is_10pct_of_spv_cash() {
        ...
        for y in 0..=10 {
            assert!(
                (bm[y].spv1_lookthrough_cash - spv1[y].crs_cash_to_investors * 0.10).abs() < 1.0
            );
    ...
    fn y10_portfolio_nav_combines_spv1_spv2() {
        ...
        let expected =
            (spv1[10].residual_nav + spv2[10].pclp1_nav_total + spv2[10].wcp_holding_fv) * 0.10;
        assert!(
            (y10.portfolio_nav - expected).abs() < 1.0,
            "BM Y10 NAV {} should be ~{}",
            y10.portfolio_nav,
            expected
        );
    }
