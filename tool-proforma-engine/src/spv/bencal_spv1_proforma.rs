...
pub fn forecast(wcp: &[WcpYear]) -> Vec<BencalSpv1Year> {
    ...
    let mut prev_shares_held = SPV1_WCP_TOTAL;
    let mut cumulative_cash: f64 = 0.0;
    let mut reserve = 0.0;

    for y in 0u32..=10 {
        let commission = if y == 0 { SPV1_COMMISSION_REBATE_Y0_GROSS } else { 0.0 };
        ...
        let shares_held = if y < 4 { SPV1_WCP_TOTAL } else { SPV1_Y10_RESIDUAL_SHARES };
        ...
        prev_shares_held = shares_held;
        let _ = prev_shares_held;
    }
    years
}
...
mod tests {
    ...
    fn y4_crs_event_distributes_cash() {
        ...
        let y4 = &spv1[4];
        assert!(y4.crs_cash_to_investors > 0.0,
                "Y4 CRS should distribute cash, got {}", y4.crs_cash_to_investors);
    }
    ...
    fn y1_y3_no_crs() {
        ...
        let spv1 = forecast(&wcp);
        for y in 1..=3 {
            assert_eq!(spv1[y].crs_cash_to_investors, 0.0,
                       "Y{} should not have CRS event", y);
        }
    }
    ...
    fn y10_residual_nav_positive() {
        ...
        let spv1 = forecast(&wcp);
        let y10 = &spv1[10];
        assert!(y10.residual_nav > 0.0,
                "Y10 residual NAV should be positive, got {}", y10.residual_nav);
