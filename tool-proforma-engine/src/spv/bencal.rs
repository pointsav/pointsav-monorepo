...
pub fn derive(wcp: &WcpData, pclp: &Pclp1Data) -> WcpData {
    ...
    let commission: [f64; 10] = std::array::from_fn(|y| {
        if y < 3 { COMMISSION_PER_YEAR } else { 0.0 }
    });
    ...
    let income = WcpIncome {
        gross_income: std::array::from_fn(|y| {
            wcp.income.gross_income[y] * wcp_sf
                + pclp.years[y].noi * pclp_sf
                + commission[y]
        }),
