...
# Institutional Brutalism: High-Fidelity Incremental CRM Engine (14-Column Provenance)
...
if [ ! -d "$VAULT_NEW" ]; then
    echo "FATAL: Staging vault not found at $VAULT_NEW"
    exit 1
fi

echo "SYSTEM EVENT: Initiating Incremental Provenance Extraction..."

cat << 'PY_EOF' > extract_eml_crm.py
...
if os.path.exists(crm_path):
    with open(crm_path, 'r') as f:
        ...
        for row in reader:
            if len(row) >= 14:
                e_addr = row[0].lower().strip()
                contacts[e_addr] = {
                    'name': row[1], 'count': int(row[2]) if row[2].isdigit() else 0,
                    'last_contact': row[3], 'phone': row[4], 'company': row[5],
                    'website': row[6], 'city': row[7], 'state': row[8],
                    'postal': row[9], 'country': row[10], 'origin': row[11],
                    'owners': set(row[12].split(' | ')) if row[12] else set(),
                    'folders': set(row[13].split(' | ')) if row[13] else set()
                }
...
PROCESSED=$(python extract_eml_crm.py "$VAULT_NEW" "$CRM_PATH" "$ROSTER_PATH" "$ARCHIVE_OWNER")
