...
fn build_library_payload(crate_name: &str) {
    println!(
        "[*] Invoking cross-compiler for Root-Task: {} (staticlib)",
        crate_name
    );
...
fn build_baremetal_payload(crate_name: &str) {
    println!(
        "[*] Invoking cross-compiler for Unikernel: {} (ELF)",
        crate_name
    );
    let status = Command::new("cargo")
        .args([
...
fn assemble_and_route(deploy_dir: &str) {
    ...
    fs::copy(kernel_src, format!("{}/kernel.elf", deploy_dir))
        .expect("[-] FATAL: Kernel routing failed.");
    fs::copy(
        root_task_src,
        format!("{}/libsystem_security.a", deploy_dir),
    )
    .expect("[-] FATAL: Root-Task routing failed.");
    fs::copy(unikernel_src, format!("{}/service-fs.elf", deploy_dir))
        .expect("[-] FATAL: Unikernel routing failed.");
