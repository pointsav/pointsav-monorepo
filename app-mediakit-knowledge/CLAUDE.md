...
**Active.** Original phases 1, 1.1, 2 (Steps 1–7; collab removed in
plan rebase), 3 (Steps 3.1–3.4), 4 (Steps 4.1–4.8), and Phase 5 core
shipped. Phase 5.1+ (per-page ACLs, OIDC SSO, webhooks) deferred
pending BP5 clearance.

KNOWLEDGE-PLATFORM-PLAN.md phases (2026-05-22 rebase):
Phases 1 (crate hygiene + collab removal), 3 (home-page iteration 1),
4 (DTCG token wiring + WCAG audit), and 5 (bilingual /es/ routing)
shipped. Phase 6 (three-instance deployment split) gated on
...
| 2 — edit | Shipped (Steps 1-7; collab removed) | JSON-LD, atomic edit, CodeMirror 6, SAA squiggles, citation autocomplete; real-time collab (yjs) removed in Phase 1 cleanup |
...
└── scripts/
    ├── dtcg-bundle.json         DTCG canonical token vault
    └── dtcg-to-css.py           Token → CSS generator (Phase 4)
```
