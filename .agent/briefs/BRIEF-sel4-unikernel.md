...
### Phase H2a — Rust rootserver (next, Day 1)

**Goal:** moonshot-sel4-vmm Rust code actually executes as the seL4 PD — not the C
shim. Proves the Rust ABI wrappers work end-to-end.

**What changes:**
1. Add `--target aarch64-unknown-none` support to moonshot-toolkit CompilePd step.
   Current CompilePd only supports C (calls aarch64-linux-gnu-gcc). Need a Rust path:
   `cargo build --target aarch64-unknown-none --release -p moonshot-sel4-vmm`
   and then link as a bare-metal ELF with no crt0.
2. Add a `_start` symbol to moonshot-sel4-vmm (AArch64 asm entry point that sets up
   stack and calls `console_main()`).
3. Add `console_main()` in moonshot-sel4-vmm that uses `debug::puts_line()` to print
   a banner and then `debug::spin()`.
4. New TOML spec: `moonshot-toolkit/examples/os-console-rust.toml` pointing at the
   Rust binary instead of console_hello.c.

**Gate:** "Hello from moonshot-sel4-vmm (Rust)" on QEMU serial. No C in the PD.

**Estimated effort:** 3-5 hours. Requires adding `[target.aarch64-unknown-none]` to
moonshot-toolkit's build logic (src/main.rs `execute_step`).

**Blocker to check:** `aarch64-unknown-none` target may need `rustup target add
aarch64-unknown-none`. Verify available before starting.
...
- H2a first: get Rust code running as the PD before adding IPC complexity
- Check `rustup target list --installed | grep aarch64-unknown-none` before starting H2a
- Stage 6 pending: Phase H1 commits (`e2dd8d70`, `9b0eb5a7`) + Phase 10 queue through
  `fc4d0978` — route to Command Session
