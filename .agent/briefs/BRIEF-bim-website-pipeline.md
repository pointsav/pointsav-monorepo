...
> **SUPERSEDED 2026-07-09.** This brief describes the pre-redesign
> `app-orchestration-bim` v0.0.2 service (port 9096, `/furniture` and
> `/key-plans` routes) as it existed on 2026-06-03. That service and its
> routes have since been replaced by the `app-privategit-bim` v0.1.0+ CMS
> rewrite, live on `bim.woodfinegroup.com` since 2026-07-08. **The current
> record for this surface is [`BRIEF-app-privategit-bim.md`](BRIEF-app-privategit-bim.md).**
> Kept here as historical record only — do not treat any content below as
> current state. Reclaimed from a misfile in `project-editorial`'s briefs
> directory (flagged via inbox `command-20260709-1-brief-file-misfiled-in-project-editori`).
