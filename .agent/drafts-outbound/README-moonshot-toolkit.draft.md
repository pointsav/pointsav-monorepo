...
references:
  - af6073f (moonshot-toolkit src/main.rs CLI rewrite)
  - 045e5cc (moonshot-toolkit src/spec.rs SystemSpec parser)
  - 59d1fc0 (moonshot-toolkit src/plan.rs BuildPlan generator)
  - ba34cd8 (moonshot-toolkit activation commit)
  - clones/project-system/pointsav-monorepo/moonshot-toolkit/src/main.rs
  - clones/project-system/pointsav-monorepo/moonshot-toolkit/src/spec.rs
  - clones/project-system/pointsav-monorepo/moonshot-toolkit/src/plan.rs
  - clones/project-system/pointsav-monorepo/moonshot-toolkit/Cargo.toml
  - clones/project-system/pointsav-monorepo/moonshot-toolkit/CLAUDE.md
  - DOCTRINE.md claims #33 + #34
  - conventions/system-substrate-doctrine.md §6
  - MEMO-2026-03-30-Development-Overview-V8.md §7
notes_for_editor: |
  Bulk README refresh per v0.1.31 Reverse-Funnel pattern. Replaces the
  activation-era README that predates the Phase 1B CLI rewrite. Project
  shipped: clap-based validate/plan/build CLI, SystemSpec TOML parser
  (Microkit-equivalent), BuildPlan deterministic generator. v0.1.x scope
  closed; #14 FUTURE for actual seL4 cross-compile.

  Editorial calls made during bulk draft:
  - "Microkit-equivalent" used as a technical reference, not brand comparison —
    retained for technical accuracy; project-language may reframe if needed.
  - "content-addressed" and "deterministic" used throughout per the actual
    technical property the crate implements; these are accurate, not marketing.
  - "bootable image" used for the output artefact; actual seL4 image format
    decisions deferred — project-language may want to hedge with "intended to
    produce" since build subcommand is currently stub.
  - Commit hashes cited directly in this frontmatter; inline prose does not
    repeat raw hashes — reader navigates via git log.
  - FUTURE task #14 called out explicitly in two sections (Status, What's
    Deferred) — bulk discipline writes both; project-language trims one.
---
