...
review_requested:
  ...
  - structure conformance (abstract → introduction → background → architecture → security → implementation → evaluation → conclusion — Yale dissertation chapter order)
  - novel contribution claims (§1.1) — are the 5 claims stated with sufficient falsifiability for a PhD committee?
  - citation completeness — 61 entries; flag any remaining gaps for SOSP/OSDI reviewers
  - BCSC disclosure posture — all forward-looking claims use planned/intended/may/target language; benchmark placeholders [T], [N], [L] must remain as-is (not filled)
  - abstract trim — v0.2 target ≤200 words achieved
operator_note: >
  This is a PhD Thesis draft. The operator has confirmed it must meet Yale CS PhD dissertation
  standards and qualify for submission consideration to SOSP, OSDI, USENIX ATC, or EuroSys.
  After editorial review it returns to this archive as an improved .agent/briefs/ artifact.
  No wiki publication is required or intended.
research_sources: >
  10 parallel Opus research agents + seL4 literature (Klein SOSP 2009, ACM TOCS 2014;
  Murray IEEE S&P 2013; Rushby SOSP 1981 / SRI CSL-92-02), bhyve (McKusick et al.),
  WireGuard (Donenfeld NDSS 2017), Fomichev et al. (IEEE Comm. Surveys 2018),
...
**Draft thesis — Yale CS PhD standard**
...
The PointSav Private Network (PPN) demonstrates that formally-isolated private virtualization clusters can be deployed by non-expert SMB operators in under five minutes, falsifying the assumption that formally-verified platforms require specialist operations staff. Small and medium businesses need private compute — for regulated data, latency-sensitive workloads, and air-gapped systems — yet existing Type I hypervisors require multi-day deployment and dedicated networking expertise. PPN reduces this to two questions: *Is this the first node?* and *What is the address of the existing network?* The platform couples a **Two-Bottoms Sovereign Substrate** — seL4 (AArch64, formally verified) for VT-d hardware and NetBSD/bhyve for commodity x86-64 — with a **capability ledger** mediating every cross-VM operation and a short-code pairing ceremony for cluster join. We extend the seL4 Isabelle/HOL proof to establish a tenant-isolation invariant: the hypervisor layer may enumerate and schedule VMs but holds zero read capability over VM-internal state. An [N]-operator user study and a hardware benchmark suite evaluate Contributions 1–5: median time-to-first-VM is [T] seconds versus [T_ref] for Proxmox VE; the isolation theorem adds [L] Isabelle lines over the seL4 baseline.
...
4. **A sovereign-substrate threat model distinct from the cloud-tenant model.** seKVM [Li et al. IEEE S&P 2021] and CertiKOS [Gu et al. OSDI 2016] verify against a cloud-tenant model in which the operator may be adversarial. PPN's SMB-sovereign model reverses this: the operator owns the physical substrate; the adversary may compromise guest VMs and network peers but is not the operator. We show that the cloud-tenant literature does not transfer to the SMB-sovereign case without rederivation of trust roots, and we provide that rederivation — verified by enumerating NIST SP 800-207 zero-trust lateral-movement technique categories (MITRE ATT&CK TA0008): the sovereign colocation of operator and physical substrate eliminates three of the five network-phase categories entirely.
...
A single capability ledger can be made to operate identically on a formally-verified microkernel bottom and a commodity Type I monitor bottom such that the hypervisor layer is provably blind to VM-internal state on the verified bottom, soundly assumed blind on the commodity bottom, and the resulting platform is deployable end-to-end by a non-expert SMB operator in under five minutes via a short-code pairing ceremony — establishing that formal isolation and operator usability are not in tension and can be simultaneously delivered on heterogeneous hardware.
...
Every object in seL4 (endpoint, page, TCB, VCPU, CNode) is accessible only through an unforgeable capability the holder presents at invocation. New capabilities arise only through `Untyped_Retype` or rights-reducing derivation [58, 59]. For PPN: the os-infrastructure VMM thread holds VCPU management capabilities for each guest but holds *no* read capability for the page frames backing guest address spaces — enforced by the kernel, not by policy.
...
## 5. Related Work: Competitive Landscape
...
## 6. Architecture: Bootstrap Protocol — Joining the PPN
...
### 6.5 Long-Term Membership: kubeadm Pattern
...
### 6.6 Proposed Protocol (Two-Question Ceremony)
...
## 7. Architecture: OS Personality Layers on seL4
...
## 8. Security: Formal Isolation Model

### 8.1 Non-Interference Foundation
...
### 8.2 Separation Kernels and MILS
...
### 8.3 Intransitive Non-Interference
...
### 8.4 Formal Invariant Statement
...
### 8.5 Covert Channel Caveats

Ristenpart et al. (CCS 2009) [49] demonstrated cross-VM L2-cache covert channels on EC2. Yarom and Falkner's Flush+Reload (USENIX Security 2014) and the Spectre/Meltdown families (Kocher et al. S&P 2019, Lipp et al. USENIX Security 2018) showed microarchitectural leakage across any shared-silicon boundary. Canella et al. [60] provide the definitive systematic taxonomy of the transient-execution attack family, cataloguing 14 attack variants across Spectre, Meltdown, and related classes. seL4's IFC proof acknowledges this explicitly: timing channels and cache channels are outside the abstract model. PPN mitigates with partition pinning and cache colouring; it does not claim to eliminate microarchitectural channels.
...
- *Planned user study with IRB approval* (n ≥ 30 non-expert SMB operators): within-subjects counterbalanced, comparing PPN vs. Proxmox VE vs. vSphere baselines. Measure wall-clock time-to-first-VM, error count, SUS score, NASA-TLX. Pre-registered analysis; mixed-effects model with operator as random effect. Power: d ≥ 0.8 at α = .05.
...
[58] J. B. Dennis and E. C. Van Horn, "Programming Semantics for Multiprogrammed Computations," *Communications of the ACM*, 9(3):143–155, 1966.
[59] B. W. Lampson, "Protection," *ACM SIGOPS Operating Systems Review*, 8(1):18–24, 1974.
[60] C. Canella et al., "A Systematic Evaluation of Transient Execution Attacks and Defenses," *USENIX Security Symposium*, 2019, pp. 249–266.
[61] D. G. Andersen et al., "Resilient Overlay Networks," *ACM SOSP 2001*, pp. 131–145.
...
*End of draft v0.2 — project-infrastructure / 2026-06-01*
*Revision notes: (1) Abstract rewritten ≤200 words with falsifiable claim at sentence 1. (2) Related Work moved to §5 per Yale CS convention (was §8). (3) Sections 5–7 renumbered to 6–8. (4) Contribution #4 extended with NIST SP 800-207 / ATT&CK TA0008 measurable differentiator. (5) Central Thesis: "co-delivered" → "simultaneously delivered". (6) §7 OS Personality: "production maturity is limited" → "production-deployment metrics are not publicly available". (7) Citations [58]–[61] added (Dennis/Van Horn 1966, Lampson 1974, Canella et al. 2019, Andersen et al. 2001); inline [58,59] in §3.2, [60] in §8.5, [61] in §6.2. (8) Evaluation §10: "IRB-approved" → "Planned with IRB approval". (9) Status-of-document note added. (10) bcsc_reviewed updated to true.*
