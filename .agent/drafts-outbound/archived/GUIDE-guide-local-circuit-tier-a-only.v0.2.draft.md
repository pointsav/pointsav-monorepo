...
status: draft
title: "Operating the Local Inference Circuit Without Tier B"
slug: guide-local-circuit-tier-a-only
target_repo: woodfine-fleet-deployment
target_path: cluster-intelligence/guide-local-circuit-tier-a-only.md
...
version: "0.2"
editorial_notes: >
  Bloomberg pass complete 2026-06-01 (project-editorial). Content is clean.
  v0.2 reflects the shipped preemption-safe watcher (binary 8b08c01d, 2026-06-01):
  Step 7 + "When Tier B returns" rewritten. Tier A fallback (Sprint 3B) is shipped
  but disabled by default. SLM_TIER_A_FIRST (Sprint 3A) remains planned/intended.
  Target path corrected from cluster-totebox-intelligence/ to cluster-intelligence/.
  Supersedes v1 placed at WFD commit 7e77081. Use bin/place-editorial.sh.
open_questions:
