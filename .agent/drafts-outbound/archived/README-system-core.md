...
Substrate-primitive crate for the Capability Ledger Substrate. Defines
the pure-data and cryptographic types every other `system-*` and
`moonshot-*` crate binds against.
...
| `Hash256` | `[u8; 32]` alias — SHA-256 baseline per WORM ledger design §3 D3 |
...
These constraints are bound by the substrate specification and must not
be changed without a doctrine MINOR:
...
- **The Capability Ledger Substrate** — constitutional anchor for
  customer-rooted Merkle-log binding. Specifies the invariants this
  crate implements: §3.1 (Capability type schema), §5.1 (Mechanism A)
- **WORM ledger design** — four-layer WORM stack; `system-core` is
  the L0 schema layer
