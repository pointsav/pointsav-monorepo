...
target_filename: guide-vm-mediakit-service-migration.md
audience: operators migrating MediaKit services from the GCP host into vm-mediakit
...
references:
  - infrastructure/virt/migrate-service-to-vm.sh
  - infrastructure/systemd/local-*.service
  - .agent/briefs/BRIEF-totebox-transformation.md §2/§10
notes_for_editor: >
  Operational runbook — English only (no .es pair required). Targets
  woodfine-fleet-deployment/fleet-infrastructure/. Covers the full migration workflow:
  running migrate-service-to-vm.sh for each service in sequence, verifying each one via
  host port-forwards, and understanding the blocked services (service-fs, bim-orch).
  The "originals stay running" principle and the no-DNS-change policy are stated explicitly
  to reassure operators that this is a test migration, not a cutover.
  TCG latency caveat (first request 30-60s) is stated so operators don't mistake slow
  startup for a service defect.
  Article frontmatter on commit: title "Migrate Services to vm-mediakit",
...
- `BRIEF-totebox-transformation.md §10` — project-data service-fs migration context
