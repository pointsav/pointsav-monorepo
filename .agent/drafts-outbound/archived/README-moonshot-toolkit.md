...
- **`plan_hash`** — SHA-256 of the canonical JSON serialisation of
  ...
  commits to per the Capability Ledger Substrate specification §6.1.
  Same spec on any machine on any date produces the same `plan_hash`.
...
not be violated without a substrate-specification revision:
...
- **Capability Ledger Substrate** — constitutional anchor for
  customer-rooted Merkle-log binding. The `plan_hash` is the artefact
  that enters the ledger; the customer-apex cosignature on that hash
  enables reproducible verification on customer metal.
- **Two-Bottoms Sovereign Substrate** — the customer-apex cosignature
  model that the `plan_hash` participates in.
- **Capability Ledger Substrate specification §6** — Reproducible-
