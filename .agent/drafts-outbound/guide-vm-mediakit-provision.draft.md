...
target_filename: guide-vm-mediakit-provision.md
audience: operators provisioning the vm-mediakit Ubuntu 24.04 guest VM on a GCP workspace host
...
references:
  ...
  - infrastructure/virt/cloud-init-mediakit/user-data
  - infrastructure/local-vm-mediakit/vm-mediakit.service
  - .agent/briefs/BRIEF-totebox-transformation.md §2/§9
notes_for_editor: >
  Operational runbook — English only (no .es pair required). Targets
  woodfine-fleet-deployment/fleet-infrastructure/. Covers provisioning the Ubuntu 24.04
  QEMU/TCG guest from scratch: prerequisites, running provision-vm-mediakit.sh, waiting
  for cloud-init, installing packages, and verifying the VM is healthy. The TCG
  performance caveat (10x slower than KVM, first HTTP request 30-60s) is stated plainly
  so operators are not confused by slow responses.
  Article frontmatter on commit: title "Provision vm-mediakit", category "operations",
  status "active", quality "review".
...
```bash
ssh-keygen -f '/home/mathew/.ssh/known_hosts' -R '[localhost]:10022'
```
...
- `BRIEF-totebox-transformation.md §2/§9` — three-VM layout and seL4 Phase 3 roadmap
