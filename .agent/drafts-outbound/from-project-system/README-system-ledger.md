...
Substrate primitives defined in `system-core`. The data types —
`Capability`, `WitnessRecord`, `SignedCheckpoint`, `LedgerAnchor`,
`InclusionProof` — live in `system-core`. This crate owns the state
machine that composes them into a kernel-side verifier.
...
- WORM tile storage — that is `service-fs` per WORM ledger design §2.
...
## IV. The N+3+ apex-handover ceremony

`system-ledger` implements the ownership-transfer ceremony specified in
the Capability Ledger Substrate specification §4. The ceremony is a
...
- Sibling crate `../system-core/` — owns the data primitives this crate
  consumes.
- **Capability Ledger Substrate** — constitutional anchor for
  customer-rooted Merkle-log binding. Specifies the invariants this
  crate implements: §3.1 (kernel binding), §4 (apex co-signing
  ownership transfer), §5 (Time-Bound Capabilities).
- WORM ledger design §2 — four-layer stack; `system-ledger` is the
  substrate-tier L1+L2 consumer, parallel to `service-fs` at the
  application tier.
- `topic-merkle-proofs-as-substrate-primitive.md` (published,
