...
## 15. Closed-loop substrate (2026-05-18)

Shipped overnight 2026-05-18 across 9 signed commits per
`.agent/plans/learning-loop-master-plan-2026-05-18.md`. The substrate
closes the capture → drain → human-gate → train → eval → promote → serve
loop named in Doctrine claim #32 (apprenticeship) and #44 (graph as
grounding surface).
...
|---|---|---|
| `slm-doorman/src/corpus_gate.rs` | Second-layer write-time quality gate (dedup, max-diff, BCSC flag, Do-Not-Use reject) | P1-1.1 |
| `slm-doorman/src/adapter_registry.rs` | Versioned LoRA adapter manifest (data/adapters/registry.yaml) | P3-3.4 |
| `slm-doorman/src/cost_ledger.rs` | Daily per-response cost rollup (data/cost-ledger/<date>.jsonl) | P3-3.5 |
| `slm-doorman-server/src/metrics.rs` | Prometheus recorder + `/metrics` endpoint | P3-3.1 |
| `service-content/src/citations.rs` | citations.yaml resolver + `/v1/citations/resolve` | P2-2.4 |
| `scripts/corpus-snapshot.sh` | Freeze corpus as zstd tarball + sha256 manifest | P1-1.9 |
| `scripts/export-dpo.sh` | Walk apprenticeship corpus, emit DPO pairs | P1-1.9 |
| `scripts/lora-update.sh` | Orchestrate snapshot → DPO → trainer → eval → registry | P1-1.9 |
| `scripts/eval-prepare.sh` | Curate eval-holdout candidates | P1-1.2-prep |
| `compute/systemd/lora-update.{timer,service}` | Weekly trigger, disabled-by-default | P1-1.9 |
| `docs/runbook-corpus-contamination.md` | Burn-and-restart procedure | P3-3.6 |

### 15.2 Tier-C contamination guards

Four defense layers, in order:

1. **Routing invariant** — `apprenticeship::pick_tier_for_brief()` returns
   only `Tier::Local` or `Tier::Yoyo`. Apprentice attempts cannot
   structurally route to Tier C.
2. **Enqueue gate** — `/v1/shadow` handler returns 403 FORBIDDEN when
   `source_tier == "external"` on the wire.
3. **Dispatch gate** — `write_shadow_tuple()` early-returns Ok(()) with a
   `contamination_guard` warn log when `attempt.tier == Tier::External`.
   The shadow JSONL file is never written.
4. **Audit replay surface** — every tuple carries `tier_used` as a
   top-level JSONL field so `jq 'select(.tier_used=="external")'` is
   O(n) without parsing the nested `attempt` subobject.

These layers hold even though Tier C is intentionally UNCONFIGURED in
production per operator directive 2026-05-18 (Claude Pro Max 20x is the
operator-side Claude path; the Commercial API key is absent).

### 15.3 Adapter version threading

`ComputeRequest.adapter_version: Option<String>` is a HINT — the backend
may not honour it. `ComputeResponse.adapter_version: Option<String>` is
GROUND TRUTH. Both skip-serialise when None.

Tier B parses `X-Foundry-Adapter-Version` from the upstream response
header. Tier A reports `None` (llama-server hot-swap deferred). Tier C
reports `None`.

`AuditEntry` + `ExtractionAuditEntry` carry adapter_version through to
the JSONL audit ledger. Prometheus emits
`slm_requests_total{adapter_version=...}` so retrospective comparison
queries become metric-series grouping.

### 15.4 Corpus quality gate (two layers)

| Layer | Where | Trigger |
|---|---|---|
| Enqueue | `queue::quality_gate_shadow` | `/v1/shadow` handler |
| Write | `corpus_gate::check` | `write_shadow_tuple` + `write_dpo_pair` |

The write gate adds:
- **Max diff cap** — 50,000 chars (~1000 LOC)
- **Dedup** — sha256 over (brief.body, actual_diff) via sidecar JSONL
  index at `data/training-corpus/.corpus-index.jsonl`
- **BCSC posture scan** — flag-only on "Sovereign Data Foundation"
  without a forward-looking marker
- **Do-Not-Use scan** — reject on POINTSAV-Project-Instructions.md §5
  terms (placeholder list pending editorial ratification)

Gate outcome embedded in JSONL as
`corpus_gate: {brief_hash, diff_hash, bcsc_flagged, bcsc_violations}`.

### 15.5 Observability

`metrics-exporter-prometheus` installed via
`slm_doorman_server::metrics::init()` at startup. `/metrics` endpoint
returns the textual format.

Counters emitted from `router::write_audit`:
- `slm_requests_total{tier,model,adapter_version,completion_status}`
- `slm_cost_usd_total{tier,model}`
- `slm_latency_ms{tier,model}` (histogram)
- `slm_audit_writes_total{entry_type}`

### 15.6 Cost ledger + adapter registry

`data/cost-ledger/<YYYY-MM-DD>.jsonl` records per-response cost rows.
`CostLedger::daily_rollup(date)` returns per-tier + per-model totals.
Tier-C kill-switch deferred (no Commercial API key in production).

`data/adapters/registry.yaml` is the versioned adapter manifest.
`AdapterRegistry::{open, from_env, get, list, promoted, append,
set_stage}`. Stages: `eval_pending → eval_ok → promoted → retired/rejected`.
Sigstore signature field reserved (P3-3.4-followup).

### 15.7 LoRA training pipeline

**HARD DISABLED** by default. Three operator-controlled gates:
1. `SLM_LORA_AUTO_ENABLE=true` (env)
2. `data/training-approved/<adapter-id>.tag` (operator ssh-signed)
3. Yo-Yo trainer provisioned + reachable

`scripts/lora-update.sh` orchestrates snapshot → DPO → LIMA gate →
trainer dispatch → eval → registry append. systemd
`lora-update.timer` ships with empty `WantedBy=` so
`daemon-reload` leaves it disabled.

### 15.8 Capture from /v1/messages

P1-1.8 wires the Anthropic shim handler → `enqueue_shadow` for SDK
clients pointing at the Doorman. Gated by `SLM_SHIM_TRAINING_CAPTURE=true`
(default off) + `state.apprenticeship.is_some()` + `tier_used !=
External`. `ShadowQueueEntry.source_tier` carries provenance for the
contamination guard contract.

Claude Pro Max sessions do NOT transit Doorman — they use the
operator's local Claude Code session directly. Capture for those is
the git post-commit hook (`bin/capture-edit.py`, already installed).

### 15.9 graph_context on apprenticeship JSONL

`dispatch_shadow` queries `GraphContextClient.fetch_context(module_id,
brief.body, 5)` before the route call and embeds the result in the
JSONL as `graph_context: <string-or-null>`. Closes the co-evolution
leg of Doctrine claim #44 — LoRA learns
`(brief + graph_context → citation-grounded prose)`.

### 15.10 What's NOT in the closed loop (deferred)

| Item | Status |
|---|---|
| F12 corpus promotion (`_review/` subdir) | P1-1.4 deferred (design-review pending) |
| Tool-use round-trip | P1-1.7 deferred (~300 LOC across 3 tier clients) |
| RelatedTo edges + neighbors endpoint | P2-2.2 deferred (editorial taxonomy) |
| Editorial seed endpoint | P2-2.3 deferred (depends on P2-2.2 + P2-2.1 wiring) |
| Embeddings + vector index | P2-2.8 deferred (sqlite-vec dep + Yo-Yo embedder) |
| Adapter A/B harness | P3-3.3 deferred (lower priority since Tier C off) |
| Canary task set | P3-3.2 deferred |
| Sigstore adapter signing | P3-3.4-followup (operator key setup) |
| Editorial grammar endpoint | P2-2.6 deferred (editorial vocab ratification) |
| Deprecate `/v1/draft/generate` | P2-2.7 stretch |

See `.agent/plans/learning-loop-master-plan-2026-05-18.md` for the
sequenced TODO.
