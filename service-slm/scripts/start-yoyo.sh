...
for ZONE in "${ZONES_TO_TRY[@]}"; do
    ...
    if [[ $? -eq 0 ]]; then
        ...
        echo "  1. Upload weights: gcloud compute scp <weights.gguf> ${INSTANCE}:/data/weights/ --zone=${STARTED_ZONE} --project=${PROJECT} --tunnel-through-iap"
        echo "  2. SSH and mount disk: gcloud compute ssh ${INSTANCE} --zone=${STARTED_ZONE} --project=${PROJECT} --tunnel-through-iap"
        echo "     Then: sudo mkfs.ext4 /dev/disk/by-id/google-yoyo-weights && sudo mkdir -p /data && sudo mount /dev/disk/by-id/google-yoyo-weights /data"
        echo "  3. Restart vllm: sudo systemctl restart vllm.service"
        echo "  4. Update SLM_YOYO_ENDPOINT in ${DOORMAN_ENV} with new internal IP if changed."
        echo "  5. Restart Doorman: sudo systemctl restart local-doorman.service"
        exit 0
    fi
done
