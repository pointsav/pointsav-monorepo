...
// Blanket impl removed: `impl<T> From<T> for ColumnOperation<T>` triggers E0119
// under rustc 1.85+ coherence checks (conflict with time crate). All call sites
// in this crate use ColumnOperation::Value(x) directly, so the impl is unused.
macro_rules! impl_column_operation_from {
    ($($t:ty),*) => {
        $(impl From<$t> for ColumnOperation<$t> {
...
impl_column_operation_from!(bool, u32, NumericalValue, UnorderedId, Ipv6Addr);
