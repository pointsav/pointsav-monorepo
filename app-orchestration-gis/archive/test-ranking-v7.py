...
def evaluate_tier(anchor_id, nhw, nwh, nhc, nhe, cont):
    ...
    # T4: Anchor + (HW or WH) + Hospital + University
    # T3: Anchor + HW + WH + (Hospital OR University) OR Anchor + (HW or WH) + (Hospital AND University)
    # T2: Anchor + HW + WH (No Civic) OR Anchor + (HW or WH) + (Hospital OR University)
    # T1: Anchor + (HW or WH)
    
    if has_hw and has_wh and has_hc and has_he: return 4
    # EU adjustment: allow 1 retail + 2 civic to be T4
    if (has_hw or has_wh) and has_hc and has_he: return 4
    
    if has_hw and has_wh and (has_hc or has_he): return 3
    
    if has_hw and has_wh: return 2
    if (has_hw or has_wh) and (has_hc or has_he): return 2
