...
def evaluate_tier(anchor_id, nhw, nwh, nhc, nhe, cont):
    ...
    # Apex (Tier 4): Alpha Anchor + HW + WH + (Med AND Edu)
    if has_alpha_anchor and has_hw and has_wh and has_hc and has_he:
        return 4
        
    # Hub (Tier 3): Alpha Anchor + HW + WH + (Med OR Edu)  [Loosened Civic]
    if has_alpha_anchor and has_hw and has_wh and (has_hc or has_he):
        return 3
        
    # Core (Tier 2): Alpha Anchor + HW + WH (No Civic) 
    # OR Alpha Anchor + (HW OR WH) + (Med OR Edu)
    if (has_alpha_anchor and has_hw and has_wh):
        return 2
    if has_alpha_anchor and (has_hw or has_wh) and (has_hc or has_he):
        return 2
    
    # Baseline (Tier 1): Alpha Anchor + (HW OR WH OR Med OR Edu)
    if has_alpha_anchor and (has_hw or has_wh or has_hc or has_he):
        return 1
