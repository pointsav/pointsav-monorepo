...
# Define the Alpha Brands per region (Carrefour elevated to EU Alpha)
ALPHA_ANCHORS = {
...
def evaluate_tier(anchor_id, nhw, nwh, nhc, nhe, cont):
    ...
    # Apex (Tier 4): Alpha Anchor + (HW OR WH) + (Med AND Edu)
    # Rationale: In Europe, Hardware & Warehouse don't cluster together perfectly as often.
    # To hit 400 sites, an Alpha Anchor with ONE secondary and BOTH civics is top tier.
    if has_alpha_anchor and (has_hw or has_wh) and has_hc and has_he:
        return 4
        
    # Hub (Tier 3): Alpha Anchor + (HW OR WH) + (Med OR Edu)
    if has_alpha_anchor and (has_hw or has_wh) and (has_hc or has_he):
        return 3
        
    # Core (Tier 2): Alpha Anchor + HW + WH (Pure Retail, no civic)
    # OR Alpha Anchor + (Med OR Edu)
    if (has_alpha_anchor and has_hw and has_wh) or (has_alpha_anchor and (has_hc or has_he)):
        return 2
    
    # Baseline (Tier 1): Alpha Anchor + (HW OR WH)
    if has_alpha_anchor and (has_hw or has_wh):
        return 1
