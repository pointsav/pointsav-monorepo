...
def evaluate_tier(anchor_id, nhw, nwh, nhc, nhe, cont):
    ...
    # We need a bell curve/pyramid. T4 should be elite (~5-10%), T3 strong (~20%), T2 common (~40%), T1 baseline (~30%)
    
    # T4: Anchor + (HW OR WH) + Hospital + University (Elite micro-city)
    if (has_hw or has_wh) and has_hc and has_he: return 4
    
    # T3: Anchor + HW + WH (Pure Destination Hub) 
    # OR Anchor + (HW OR WH) + (Hospital OR University)
    if has_hw and has_wh: return 3
    if (has_hw or has_wh) and (has_hc or has_he): return 3
    
    # T2: Anchor + HW (The classic pair)
    if has_hw: return 2
