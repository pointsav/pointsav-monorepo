...
def evaluate_tier(anchor_id, nhw, nwh, nhc, nhe, cont):
    ...
    # T4: Elite. Anchor + HW + WH + Civic (Both) -> Actually let's use Anchor + (HW or WH) + Civic(Both)
    # T3: Hub. Anchor + HW + WH -> OR Anchor + (HW or WH) + Civic(One)
    # T2: Core. Anchor + (HW or WH)
    # T1: Baseline. Anchor + Civic(One)
    
    if (has_hw or has_wh) and has_hc and has_he: return 4
    if (has_hw and has_wh) or ((has_hw or has_wh) and (has_hc or has_he)): return 3
    if has_hw or has_wh: return 2
    if has_hc or has_he: return 1
