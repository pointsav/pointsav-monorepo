#!/usr/bin/env python3
"""
generate-top400.py — Top-400 co-location index from clusters.geojson (§2 schema).
...
Produces continent-separate Top-400 slices:
    NA: continent == "NA"
    EU: continent == "EU"

Each record carries the Regional Market column so downstream editorial
can group by RM when building the Top-400 wiki pages.

Reads:  work/clusters.geojson
Writes: work/top400_final.json
"""
import json
import sys
...
OUTPUT_FILE = WORK_DIR / "top400_final.json"
