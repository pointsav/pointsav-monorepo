...
- Nothing in progress. Phase 1C.d complete (v0.3.0, 2026-05-29): `AssembleImage`
  is a full Rust orchestration. `moonshot-toolkit build` produces a bootable
  `build/system-image.bin`. QEMU boot verified to user space.

## Queue

- **`build-totebox.sh` removal** — kept as migration reference; Phase 1C.d complete
  so this can now be removed. [2026-totebox]
...
- **hello.c rootserver print** — `examples/hello.c` halts in an infinite loop;
  wire `SysDebugPutChar` to produce observable output from user space. [minor]
