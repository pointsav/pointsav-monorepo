...
async fn render_index(state: &AppState, lang: Lang) -> Html<String> {
    ...
    let (eyebrow, h1, lede1, lede2_pre, cta_text, stat_components_label, stat_tokens_label, tag_dtcg, tag_apache,
        ...
        Lang::En => (
            ...
            "Eleven document families — subscription agreements, prospectuses, financial reports, PDF-binder navigation, trust &amp; offering instruments — share this same rule-weight ladder.",
        ...
        Lang::Es => (
            ...
            "Once familias de documentos — acuerdos de suscripción, prospectos, informes financieros, navegación de carpetas PDF, instrumentos de fideicomiso y de colocación — comparten esta misma escala de grosor de regla.",
...
&nbsp;&nbsp;<span class=\"curl-key\">\"tokens\"</span>: [<span class=\"curl-str\">\"{{semantic.interactive-primary}}\"</span>, <span class=\"curl-str\">\"{{semantic.interactive-primary-hover}}\"</span>]\n\
    ...
    let (title, description, path) = match lang {
        ...
        Lang::Es => (
            ...
            "Sistema de diseño de PointSav — tokens, componentes y documentación de investigación que abarcan el lenguaje visual y los primitivos de interfaz de usuario.",
            "/es",
        ),
    };
...
pub async fn tokens_gallery_page(State(state): State<AppState>) -> Html<String> {
    ...
    Html(render::shell(
        ...
        "The PointSav Design System's token registry — primitive and theme-level DTCG tokens covering visual language and UI primitives.",
        "/tokens",
        &PageLang::en_only(),
        &nav_html,
        "",
...
pub async fn adoption_page(State(state): State<AppState>) -> Html<String> {
    ...
    Html(render::shell(
        ...
        "Component adoption, grouped by consuming application, in the PointSav Design System registry.",
        "/adoption",
        &PageLang::en_only(),
        &nav_html,
        "",
...
pub async fn item_tab(
    ...
    let description = frontmatter
        ...
        .unwrap_or_else(|| {
            format!(
                "{label} — {} in the PointSav Design System registry.",
                vault::to_title(&section)
            )
        });
