...
- [x] **Design commission → project-design** — 10 DESIGN-* drafts routed to project-design inbox by Command (`command-20260524-knowledge-design-routing`). MASTER COSIGN on DTCG and `shell_chrome()` implementation deferred; separate task. [2026-05-24 command@claude-code]
- [x] **Hybrid UI implemented** (`9cf2c9ed`) — design competition D+A+B hybrid: three-row shell, editorial token layer, 21st-century Wikipedia progressive disclosure, canonical footer trademark, WCAG AA, 106 tests green. Stage 6 pending (Command scope). [2026-05-24 totebox@claude-code]
- [ ] **Stage 6 — hybrid UI** — promote `9cf2c9ed` + `cargo build --release` + `sudo systemctl restart local-knowledge-{documentation,projects,corporate}` (Command scope). [2026-05-24 totebox@claude-code]
